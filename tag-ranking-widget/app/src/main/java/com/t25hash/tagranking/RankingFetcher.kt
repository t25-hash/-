...
/**
 * Qiitaは公式APIなので問題なし。
 * Zennは公式APIが無く、正解のエンドポイントが未確認なので候補を順に試す(AI_SPEC.md参照)。
...
object RankingFetcher {
    ...
    fun fetchZenn(tag: String): List<RankedArticle> {
        ...
        val candidates = listOf(
            "https://zenn.dev/api/articles?topicname=$encodedTag&order=daily",
            "https://zenn.dev/api/articles?topicname=$encodedTag&order=latest",
        )
        for (candidateUrl in candidates) {
            try {
                ...
                if (result.isNotEmpty()) {
                    return result.sortedByDescending { it.likes }
                }
            } catch (e: Exception) {
                // 次の候補へ
            }
