...
## 現状
なろう/カクヨムの実ページをWebViewで開き、本文コンテナに縦書きCSSを注入して読む、薄いアプリ。本文は一切保存・複製しない(毎回サイトから読む)。
...
## アーキテクチャ
- `MainActivity`: WebViewを1つ表示するだけの単一Activity。`shouldOverrideUrlLoading`で許可ドメイン外への遷移をブロック。
